...
# - However, to grant admin consent for all Enterprise users via Terraform,
...
# 
# Configuration:
# - app_role_assignment_required = false: All org users can access without individual assignment
...
# Client secret for external integrations (requires confidential client)
# 2 year expiry - minimum practical duration for Azure AD
resource "azuread_application_password" "external_connector" {
  application_id = azuread_application.agentcore_app.id
  display_name   = "External Agent Connector"
